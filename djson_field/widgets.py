...
def get_name_by_path(base, path):
    return base + string.join(
        ["[%s]" % (ob if isinstance(ob, int) else "_" + ob) for ob in path], ''
    )
...
class JSONWidget(forms.Textarea):
    ...
    ADD_ACTIONS = {
        "add_plain": _(u"простое значение"),
        "add_list": _(u"список"),
        "add_dict": _(u"словарь"),
    }
    ...
    def _update_media(self):
        """Update Media from subwidgets."""
        subwidgets_media = self._subwidgets_media()

        subwidgets_media.add_js(self.Media.js)
        subwidgets_media.add_css(self.Media.css)

        self.Media.css = subwidgets_media._css
        self.Media.js = subwidgets_media._js

    def _subwidgets_media(self):
        """Get additional widgets from field in rules and add their media."""
        ...
        if self.rules:
            for rule in self.rules:
                # collect only unique fields
                try:
                    if 'type' in rule[1]:
                        # rule[1]['type'] can contain dict with field_name => field
                        if isinstance(rule[1]['type'], dict):
                            for key, field in rule[1]['type'].iteritems():
                                fields.append(field)
                                fields[field.__class__.__name__] = field
                        else:
                            fields[rule[1]['type'].__class__.__name__] = rule[1]['type']
                except IndexError:
                    pass

            # collect all widgets media
            for item in fields.values():
                field_instance = item.formfield()

                if hasattr(field_instance, 'widget'):

                    media += field_instance.widget.media
    ...
    def render_field(self, name, data, path, field=None, rules=None):
        ...
        if isinstance(field, dict):
            ...
            items = OrderedDict(
                [(key, self.render_field(name, None, path + [key], ob)) for key, ob in field.iteritems()]
            )
